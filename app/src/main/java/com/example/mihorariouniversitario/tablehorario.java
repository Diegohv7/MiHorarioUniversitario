...
public class tablehorario {
    ...
    private void createDataTable(){
        String info;
        //header es la cant de columnas en la tabla
        for (indexR =1;indexR  <=header .length;indexR ++){
            newRow();
            for(indexC =0;indexC <header .length ;indexC ++){
                newCell() ;
                String [] row=data.get(indexR -1);
                info=(indexC <row.length )?row [indexC ]:"";//por si tengo mas columnas sin datos lo llena en vacio
    ...
    public void addItems(String[]item){
        ...
        tableLayout .addView(tableRow ,data.size()-1 );//inserte la fila en la ultima pos
        reColoring();//para q se pinte dependiendo de a funcion
    }
    //recibir color de fondo de la tabla
    ...
    private TableRow .LayoutParams newTableRowParams(){
        ...
        params .setMargins(1,1,1,1);//left para editar margen
        params .weight =1;
        return params ;
    }
}
